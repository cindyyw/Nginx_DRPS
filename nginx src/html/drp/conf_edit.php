<?php
	...
	/*if( !isset($_COOKIE["userName"] ) )
	{
		...
		echo "<script> location='http://www.service4all.com.cn/repository/testuser/login.jsp';</script>";
		exit;
	}
	else
	{
		if( $_COOKIE["userName"] != "user1")
		{
			...
			echo "<script> location='http://121.199.25.81:5200/drp/show.php';</script>";
			exit;
		}
	}*/

	$semID = sem_get(ftok("/usr/local/nginx-1.4.2-file/applicationList.xml",'a'),1);

	$pid="";
	$fp = fopen("/usr/local/nginx-1.4.2-file/pid","r");
	while( !feof($fp) )
	{
		$line = fgets($fp);
		$pid.= $line;
				...
				<legend>Http</legend>
					...
					<fieldset id="server_field">
						...
						<div id="server">
							...
							<div id="server_fir">
								<br />listen <input size="9" type="text" name="listen_port"  value=<?php echo '"'.trim(rcvData()).'"'; ?> >
								<br /> server_name <input size="9" type="text" name="server_name"  value=<?php echo '"'.trim(rcvData()).'"'; ?> >
								<br /> access_log logs/invokeCounts.log InvokeCounts; 
								<br />
