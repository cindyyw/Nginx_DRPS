<?php
	...
	{
		$res = 0;
		$fp = fopen("/usr/local/nginx-1.4.2-file/logs/invokeCounts.log","r");

		if($name[0] == 'W' && $name[1]=='S' && $name[2]=='_' )
		{
			$ip.="|/";
			$ip.="axis2/services/";
			$ip.=$name;
	...
	$fp = fopen("/usr/local/nginx-1.4.2-file/applicationList.xml","r+");
	...
	if( flock($fp,LOCK_SH))
	{
		...
		$doc = new DOMDocument();
		$doc->load("/usr/local/nginx-1.4.2-file/applicationList.xml");
